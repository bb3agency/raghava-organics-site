...
export function AdminPaymentsList() {
  ...
  }, [api, page, status, method, orderId, fromDate, toDate]);
        ...
        <div className="mb-4 grid gap-3 rounded-md border border-border bg-muted/20 p-3">
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              {PAYMENT_FILTER_STATUSES.map((value) => (
                ...
                <tbody>
                  {items.map((payment) => (
                    <tr key={payment.id} className="border-b border-border last:border-0">
                      ...
                      <td className="px-3 py-2 font-medium">{formatPaise(payment.amount)}</td>
                      <td className="px-3 py-2 text-xs text-muted-foreground">
                        {payment.capturedAt ? formatAdminDate(payment.capturedAt) : "—"}
                      </td>
                      <td className="px-3 py-2">
                        <button
                          type="button"
      ...
      >
        {detailLoading ? (
          ...
          <dl className="grid gap-2 text-sm">
            ...
            <DetailRow
              label="Captured"
              value={detail.capturedAt ? formatAdminDate(detail.capturedAt) : "—"}
            />
