      ...
      <DetailCard
        ...
        action={
          ...
          (order.invoice?.hasPdf || isInvoiceEligibleOrderStatus(order.status)) ? (
            <button
              type="button"
              className="inline-flex h-10 items-center gap-1.5 rounded-lg border border-[#efe8e4] px-3 text-xs font-bold text-[#23403d] transition-colors hover:bg-[#faf3ef] disabled:opacity-50 sm:h-8"
      ...
      {order.isLocalDelivery && !order.shipment?.awb && (
        <DetailCard icon={<Truck className="size-4" aria-hidden />} title="Delivery">
          <p className="text-sm text-[#767676]">
            This order is delivered directly by Raghava Organics — no courier tracking
            number is issued. You&apos;ll be notified as it moves through packing, out for
