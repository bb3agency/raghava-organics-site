...
3. **Validation:**
   - Searched for other descriptive instances of "organic" (ignoring the "Raghava Organics" store name, email address domains, and technical database filter slugs) and confirmed none remain in public storefront pages.
...
- **Frontend (already correct, verified):** `AddToCartButton` persists to `POST /cart/items`; `useCartSync` reads `GET /cart`; `lib/api.ts` sends `credentials: 'include'`; browser calls the API same-origin (`window.origin + /api/v1`); login → `mergeGuestCartAfterAuth` → `POST /cart/merge`. No frontend change needed. Merge was already additive (`existing.quantity + guestItem.quantity`).
- **Verified live on prod:** same `cart_session` token now returns a stable cart id that accumulates items across GET/POST/GET (before, the id changed every request).
...
- Configured for Raghava: creds in Ops Config (not `.env`), webhook verified at `https://raghavaorganics.com/api/v1/notifications/webhook/meta-whatsapp` (apex, not `api.`), 6 UTILITY templates in WhatsApp Manager. Operator details + credentials: `docs/clients/raghava-organics/VPS_INPUTS.md` §Meta WhatsApp. Canonical template bodies: `backend/docs/WHATSAPP_TEMPLATE_REGISTRY.md`.
- To route a notification over WhatsApp, set its `primaryChannels` entry to `WHATSAPP` (default EMAIL).
