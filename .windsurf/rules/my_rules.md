...
3. **Confirm backend `.env` is correctly configured** — BEFORE running any script, verify ALL of the following in the backend `.env`:
   ...
   - `DATABASE_URL` matches `CLIENT_ID` naming (e.g. `postgresql://.../<client_db_name>`) — **not** `ecom_template`
   - `REDIS_PASSWORD` is non-empty — blank value causes `ECONNRESET` loops
   - `REDIS_URL` uses the same password as `REDIS_PASSWORD` (e.g. `redis://:yourpassword@localhost:6379`)
   - `JWT_SECRET`, `JWT_REFRESH_SECRET`, `OPS_DB_ENCRYPTION_KEY` are set to unique non-placeholder values
   - If any of the above are wrong, STOP and fix them before proceeding. Reference `README.md` §Local Development Quickstart Step 1 for the full annotated `.env` block.
