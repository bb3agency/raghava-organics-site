...
function requestBodyFor(route) {
  ...
  const bodyByRoute = {
    'POST /api/v1/auth/admin/login': { email: 'admin@example.com', password: 'Admin@12345' },
    ...
    event: [
      {
        ...
        script: {
          ...
          exec: [
            ...
            'if (!token) {',
            '  pm.sendRequest({',
            "    url: pm.environment.get('baseUrl') + '/api/v1/auth/admin/login',",
            "    method: 'POST',",
            "    header: { 'Content-Type': 'application/json' },",
            '    body: {',
            "      mode: 'raw',",
            "      raw: JSON.stringify({ email: 'admin@example.com', password: 'Admin@12345' })",
            '    }',
            '  }, (err, res) => {',
            '    if (!err && res && res.code === 200) {',
            '      const json = res.json();',
            "      pm.environment.set('adminToken', json.data.accessToken);",
            "      pm.environment.set('userId', json.data.admin.id);",
            '    }',
            '  });',
            '}',
