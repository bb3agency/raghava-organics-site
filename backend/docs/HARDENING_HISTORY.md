...
3. **Edge cutover via Nginx `auth_request`.** Backend exposes `GET /api/v1/maintenance/gate` that **always returns 200** but sets the `X-Maintenance-Active: 0|1` header based on whether `X-Original-URI` should be blocked. Nginx guarded `location` blocks call this via `auth_request /_maintenance_gate;` + `auth_request_set $maintenance_active $upstream_http_x_maintenance_active;` + `if ($maintenance_active = "1") { return 503; }`, which trips `error_page 503 /maintenance.html`. **The header-based design replaces an earlier 503-direct design** because Nginx remaps subrequest 5xx to client 500, which would have hidden genuine upstream failures and triggered the wrong error page. Explicit bypass for `/ops/*`, `/api/v1/health*`, `/api/v1/auth/*`, `/api/v1/maintenance/*`, `/api/v1/payments/webhook`, `/api/v1/shipping/webhook` — the gate skips them entirely.
