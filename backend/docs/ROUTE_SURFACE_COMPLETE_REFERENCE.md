...
- **`maskedValue`** — always present. The decrypted value passed through `maskSecretValue()` (e.g. `s••••••et` for `shiprocket`).
- **`plaintextValue`** — present **only for non-secret keys**. Classification is performed by `isOpsConfigSecretKey()` in `ops-config-contract.ts`, which mirrors the frontend `isSecretKey()` predicate exactly. Non-secret keys are operational metadata that has no security benefit from masking: provider selectors (`SHIPPING_PROVIDER`, `PAYMENT_PROVIDER`, `SMS_PROVIDER`, `EMAIL_PROVIDER`), URLs (`DELHIVERY_BASE_URL`, `SHIPROCKET_BASE_URL`), pincodes (`*_PICKUP_PINCODE`), allowlist CIDRs (`*_WEBHOOK_ALLOWLIST_CIDR`, `TRUSTED_PROXY_ALLOWLIST_CIDR`), boolean flags (`NOTIFY_*_ENABLED`, `*_FAILOVER_ENABLED`), integer thresholds (`*_MAX_SKEW_SECONDS`, `*_CB_FAILURE_THRESHOLD`, `*_CB_COOLDOWN_MS`, `REPLAY_AUDIT_RETENTION_DAYS`), public identifiers (`RAZORPAY_KEY_ID`, `MSG91_SENDER_ID`, `META_WHATSAPP_PHONE_NUMBER_ID`), sender addresses (`RESEND_FROM`), and login emails (`SHIPROCKET_EMAIL` — paired separately with `SHIPROCKET_PASSWORD` which IS masked).
- Real cryptographic secrets (`_SECRET`, `_TOKEN`, `_PASSWORD`, `_API_KEY`, `_AUTH_KEY`, `_APP_SECRET`, ops cookie secret, signed approval tokens) **never** carry `plaintextValue` — only `maskedValue`. This complies with the workspace rule *"Never show plaintext secret values in admin UI — always mask"*.
- Frontend uses `plaintextValue` to prefill the Ops Config editor input so operators see what's saved without retyping. Secret fields show `Stored: <maskedValue> — enter new value to replace` as placeholder text instead.
- Regression-guarded against `_SECONDS` vs `_SECRET` confusion (e.g. `SHIPROCKET_WEBHOOK_MAX_SKEW_SECONDS` correctly returns `plaintextValue`, NOT masked).
